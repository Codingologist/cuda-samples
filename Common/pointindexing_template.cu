...
int main(int argc, char *argv[])
{
    ...
    auto result = thrust::reduce_by_key(d_cellids, d_cellids + num_points,
                                    thrust::constant_iterator<int>(1),
                                    d_PKey, d_PLen);
    size_t num_cells = result.first - d_PKey;//num_cells is initialized to 0 just to make the template compile; it should be updated next
